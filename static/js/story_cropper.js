(function () {
  ...
  function initStoryCropper(root) {
    ...
    input.addEventListener('change', () => {
      ...
      const boot = () => {
        if (!window.Cropper) {
          // Give the CDN a moment if it is still loading.
          return false;
        }
      ...
      // Cropper works best once the image is decoded.
      try {
        img.onload = () => {
          // Attempt a few times in case cropper script is deferred.
          let tries = 0;
          const tick = () => {
            tries += 1;
            const done = boot();
    ...
    form.addEventListener(
      ...
      async (e) => {
        try {
          ...
          // Prevent HTMX/native submit until we swap the file.
          e.preventDefault();
          e.stopPropagation();
